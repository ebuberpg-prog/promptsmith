...
function defaultPromptName(text: string, labels: string[]) {
  return (text.trim() || labels.join(' ')).split(/\s+/).filter(Boolean).slice(0, 6).join(' ') || `Untitled prompt ${new Date().toLocaleDateString()}`
}
...
export function PromptOutput() {
  ...
  const suggestedName = useMemo(() => defaultPromptName(store.customText, store.selectedTags.map((tag) => tag.label)), [store.customText, store.selectedTags])
...
function PersistenceLabel({ state }: { state: 'saving' | 'saved' | 'best-effort' | 'error' }) { return <span>{state === 'saving' ? 'Saving…' : state === 'error' ? 'Save failed' : state === 'best-effort' ? 'Best-effort storage' : 'Saved on this device'}</span> }
