...
function CraftView({ taxonomy }: { taxonomy: TaxonomyCategory[] }) {
  ...
  const [openStudioSection, setOpenStudioSection] = useState('prompt-check')

  return (
    <div className={`${workspaceDepth === 'studio' ? 'max-w-7xl' : 'max-w-5xl'} mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-5`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="font-display text-3xl sm:text-4xl text-balance">Craft</h1>
        <div className="flex items-center gap-1 p-1 rounded-lg border border-[var(--ui-border)]" role="group" aria-label="Workspace depth">
          <DepthButton label="Simple" active={workspaceDepth === 'simple'} onClick={() => setWorkspaceDepth('simple')} />
          <DepthButton label="Studio" active={workspaceDepth === 'studio'} onClick={() => setWorkspaceDepth('studio')} />
        </div>
      </div>
      ...
      <div className={`${workspaceDepth === 'studio' ? 'grid xl:grid-cols-[minmax(0,1fr)_320px]' : ''} gap-5 items-start`}>
        <div className="space-y-4 min-w-0">
          <PromptOutput />
          <button type="button" onClick={() => setBrowseAll(true)} className="min-h-11 px-4 rounded-lg border border-[var(--ui-border)] text-sm text-[var(--ui-muted-text)] hover:text-[var(--ui-text)]">Add ingredients</button>
        </div>
...
function PromptCheck({ onBrowse }: { onBrowse: () => void }) {
  ...
  return <div className="space-y-3"><div className="flex items-center justify-between gap-3"><span className="text-xs text-[var(--ui-muted-text)]">Covered</span><span className="text-xs tabular-nums">{dimensions.size} / {all.length}</span></div><div className="h-1 rounded-lg bg-[var(--ui-surface-soft)] overflow-hidden" aria-hidden="true"><span className="block h-full bg-[var(--ui-text)]" style={{ width: `${(dimensions.size / all.length) * 100}%` }} /></div>{missing.length > 0 ? <div className="flex flex-wrap gap-2">{missing.map((item) => <button key={item} type="button" onClick={onBrowse} className="min-h-11 px-3 rounded-lg border border-[var(--ui-border)] text-xs capitalize">Add {item}</button>)}</div> : <p className="text-xs text-[var(--ui-muted-text)]">Core dimensions covered.</p>}</div>
}
...
function LibraryView({ taxonomy }: { taxonomy: TaxonomyCategory[] }) {
  ...
  const prompts = savedPrompts.filter((prompt) => !query.trim() || `${prompt.name} ${prompt.customText}`.toLowerCase().includes(query.toLowerCase()))
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3">
        <div className="flex gap-1 p-1 rounded-lg border border-[var(--ui-border)] overflow-x-auto" role="tablist" aria-label="Library sections">
          {(['prompts', 'templates', 'tags', 'references'] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={tab === item} onClick={() => setTab(item)} className={`min-h-11 px-4 rounded-lg text-sm capitalize ${tab === item ? 'bg-[var(--ui-text)] text-[var(--ui-bg)]' : 'text-[var(--ui-muted-text)]'}`}>{item}</button>)}
        </div>
      ...
      {tab === 'prompts' && (prompts.length === 0 ? <section className="min-h-44 flex flex-col sm:flex-row items-center justify-between gap-6 rounded-2xl border border-dashed border-[var(--ui-border)] px-6 py-7"><div className="flex items-center gap-4"><span className="size-11 rounded-full border border-[var(--ui-border)] flex items-center justify-center"><Heart className="size-5 text-[var(--ui-muted-text)]" /></span><div><h2 className="font-display text-2xl text-balance">Keep the first idea worth returning to.</h2><p className="mt-1 text-sm text-pretty text-[var(--ui-muted-text)]">Draft freely, then save it on this device.</p></div></div><button type="button" onClick={() => setWorkspaceView('home')} className="min-h-11 shrink-0 px-4 rounded-lg bg-[var(--ui-text)] text-[var(--ui-bg)] text-sm">Start a prompt</button></section> : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{prompts.map((prompt) => <LibraryPromptCard key={prompt.id} prompt={prompt} onExport={exportPrompt} />)}</div>)}
      {tab === 'templates' && <TemplateGallery />}
      {tab === 'tags' && <section className="rounded-2xl border border-[var(--ui-border)] bg-[var(--ui-surface-elevated)] p-4 sm:p-6"><div className="mb-5 max-w-2xl"><p className="text-xs text-[var(--ui-muted-text)]">Complete taxonomy</p><h2 className="font-display text-3xl text-balance">Every ingredient, organized for discovery.</h2><p className="mt-2 text-sm text-pretty text-[var(--ui-muted-text)]">Browse by subject, setting, lighting, composition, style, mood, and more. Selecting a tag adds it to the current draft.</p></div><SmartTagBrowser taxonomy={taxonomy} /></section>}
      {tab === 'references' && <ReferenceUploader layout="library" />}
    </div>
  )
}
