...
MUSE is designed as a local-first browser application. It has no account system and does not include product analytics, advertising trackers, or behavioral telemetry.

## Data stored on the device

MUSE can store the following in the current browser profile:
...
The durable workspace is stored in IndexedDB. Theme and recoverable legacy state may use local storage. Cloud-provider API keys use session storage only.
...
1. **Application loading and updates** — GitHub Pages serves the application, taxonomy, built-in inspiration, PWA manifest, and service-worker updates.
...
Users can delete individual prompts, versions, references, and other Library records in the interface. Clearing the site's browser data removes the local workspace. Because there is no MUSE account or cloud sync, cleared browser data cannot be recovered unless an exported backup exists.
